...
La contrainte académique est précise. Le site WordPress doit présenter au minimum trois pages cohérentes : une page d'accueil affichant les actualités et ajouts récents, une page de CV, et une page de réalisations listant les projets. Un projet particulier — le fil rouge 4.0.2 — doit y être détaillé avec une attention distinctive. Les compétences acquises pendant la formation (référentiels Compétences 4.0.1 et 4.0.2 de l'UHA 4.0) doivent être explicitement rattachées aux projets, pour prouver qu'elles ne sont pas théoriques mais démontrées par des réalisations. Les bonnes pratiques SEO et les plugins correspondants doivent être correctement paramétrés. Une page de contact est optionnelle. L'architecture du projet va largement au-delà de ces exigences, ce qui est assumé : on traite la contrainte comme un minimum, pas comme un plafond.
...
| API C# | .NET 8, ASP.NET Core Minimal APIs, Markdig, IMemoryCache, HttpClient |
